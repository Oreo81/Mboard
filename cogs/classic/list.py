    ...
    async def listcreate(self,ctx,*name):
        ...
        if not name:
            ...
            if name_alrdy_take(name,id_admin):
                error = await ctx.send(f"Vous avez déjà une table sous ce nom")
            else:
                ...
                creation_message = await ctx.send(f"Table {name} créé, cette list est en privé.")
                sqlconnect.commit()
    ...
    async def listdelete(self,ctx,*name):
        ...
        if not name:
            ...
            if not name_alrdy_take(name,id_admin):
                error = await ctx.send(f"Vous n'avez aucune table sous ce nom")
            else:
                ...
                try:
                    ...
                    if str(reaction.emoji) == valid_reactions[0]:
                        ...
                        await del_list_confirm.clear_reaction(u"\u274C")
                        await del_list_confirm.edit(content="List sup")

                    else:
                        await del_list_confirm.clear_reaction(u"\u2705")
                        await del_list_confirm.clear_reaction(u"\u274C")
                        await del_list_confirm.edit(content="Cancelled")

                except asyncio.TimeoutError: 
                        await del_list_confirm.clear_reaction(u"\u2705")
                        await del_list_confirm.clear_reaction(u"\u274C")
                        await del_list_confirm.edit(content="Cancelled [Timeout]")
